...
ALTER TABLE projects ADD COLUMN quota int;
ALTER TABLE private_isolates ADD COLUMN embargo date NOT NULL;

DROP INDEX seqbin_stats_n50_idx ;
...
ALTER TABLE schemes ADD COLUMN quality_metric_count_zero boolean NOT NULL DEFAULT FALSE;

CREATE INDEX ON history USING brin(timestamp);

ALTER TABLE submissions ADD COLUMN dataset text;
ALTER TABLE submissions ADD COLUMN embargo int;
...
DROP INDEX i_i_date_entered;

ALTER TABLE projects DROP COLUMN quota;
ALTER TABLE private_isolates DROP COLUMN embargo;

CREATE INDEX ON seqbin_stats(n50);
CREATE INDEX ON seqbin_stats(l50);
CREATE INDEX i_as2 ON allele_sequences (datestamp);

ALTER TABLE sequence_flags DROP CONSTRAINT sf_curator;

CREATE INDEX i_ad4 ON allele_designations (datestamp);

ALTER TABLE schemes DROP COLUMN allow_presence;
ALTER TABLE schemes DROP COLUMN quality_metric_count_zero;

DROP INDEX history_timestamp_idx;

ALTER TABLE submissions DROP COLUMN dataset;
ALTER TABLE submissions DROP COLUMN embargo;

DROP TABLE assembly_submissions;
