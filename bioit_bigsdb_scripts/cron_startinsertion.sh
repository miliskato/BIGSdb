...
do
  ...
  uploader=$(cat $dir/info.txt | grep -oPw "(?<='user': ')[^']*")
  {
    ...
    $VENV_PYTHON_BIGSDB /home/bigsdb/BIGSdb/bioit_bigsdb_scripts/main_results_inserter.py --tsvfilepath /reports/$species/$sample_name/report.tsv --isolatename $sample_name --uploader_mail_address $uploader --species $species --results_type new_isolate
