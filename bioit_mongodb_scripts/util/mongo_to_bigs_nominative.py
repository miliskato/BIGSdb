        ...
        for document in list_of_documents:
            ...
            if sample_presence[0][0] != 0:  # only add metadata if sample exists in bigsdb
                ...
                with TblIsolates(self._species) as isolates_psql_tbl:
                    isolate_update_query = isolates_psql_tbl._build_update_nomin_metadata_query(dict_to_be_inserted)
                    value_to_set_in_fields = [v for v in dict_to_be_inserted.values()]
                    value_to_set_in_fields.append(mapping_table['_id'])
