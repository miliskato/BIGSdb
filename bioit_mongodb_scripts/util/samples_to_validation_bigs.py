...
def _insert_submission_bigs(sample_docs: List[Dict[str, Any]], validation_type: str, species: str, mongo_dtap: str) -> None:
    """
    ...
    :param mongo_dtap: mongodb dtap
    :return: None
    """
    local_path = 'reports'
    azure_path = f'results/{mongo_dtap}'
    with TblSubmissions(species) as isolates_sub_psql_tbl, \
        ...
        for doc in sample_docs:
            ...
            html_path = html_path.replace(local_path,"galaxyreports")
            html_path = html_path.replace(azure_path, "galaxyreports")
            html_link = f'<p><a href="{html_path}/report.html" target="_blank"> html report</a></p>'
            # end of dev code
...
def samples_to_validation_bigs(species: str, mongo_config_data: Dict[str, Any] = None) -> None:
    ...
    _insert_submission_bigs(bad_samples, 'bad_quality', species, mongo_config_data['dtap'])
    resequencing_samples = list(isolates_resequencing_collection.find({'creation_date': {'$gt': last_run_date}}))
    _insert_submission_bigs(resequencing_samples, 'resequencing', species, mongo_config_data['dtap'])
