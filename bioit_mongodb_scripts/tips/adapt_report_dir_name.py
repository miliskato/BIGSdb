...
if __name__ == '__main__':
    ...
    new_reports = [x for x in new_reports if
                   x not in regenerated_reports]  # remove those which are already processed above
    ...
    reports_from_renamed = [x for x in other_reports if x in renamed_in_mongo]
    # reports_from_extra_cat= reports_from_bad_qc + reports_from_reseq + reports_from_old + reports_from_renamed
    # if len(other_reports) != len(reports_from_extra_cat):
    #    print(str(len(other_reports))+" expected and found "+ str(len(reports_from_extra_cat))
    ...
    for i in list_of_subfolder:
        report = i.stem
        if (report != "S21BD07114_2023-06-14"):
            try:
                report.index("2023-06-14")
                report[:report.index("2023-06-14")]
                new_fold_name = report[:report.index("2023")] + str(today)
                new_path = Path(reports_folder, new_fold_name)
