...
def _parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--fastafilepath", required=True, type=str)
    parser.add_argument("--vcffilepath", required=True, type=str)
    parser.add_argument("--technical_id", required=True, type=str)
    return parser.parse_args()

def _write_collection(opened_collection, json_input: dict):
...
if __name__ == '__main__':
    ...
    args.technical_id = "technical_test_new3"
    if args.results_type == "new_isolate":
        if args.technical_id in mongoquerying._query_list_of_all_values(isolates_collection, "_id"):
            ...
            #_write_collection(species_database, "isolate_results", records)
            _write_collection(isolates_collection, {"_id": args.technical_id, "vcf_path": args.vcffilepath, "fasta_path": args.fastafilepath, "latest_results_version": _write_collection(isolateresults_collection, records)})
