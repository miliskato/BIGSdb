...
def query_hashes_of_scheme(hashed_AD_collection, scheme: str) -> list:
    return [document for document in hashed_AD_collection.with_options(read_concern=ReadConcern(level="majority")).find({"scheme": scheme, "resolved":0})]
...
if __name__ == '__main__':
    ...
    if documents_list == []:
        ...
        for hash_document in documents_list:
            if locus_hash_dict[hash_document['locus']]:
        ...
        for locus, hash_list in hash_document.items():
