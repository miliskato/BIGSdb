...
class NewAllelesProfileClusteringFromMongoToBigs:
    ...
    def _insert_or_update_clustering(self):
        ...
        for cl_membership in self.new_cluster_membership:
            ...
            self.cur_seqdef.execute(
                f"SELECT * FROM classification_groups WHERE (cg_scheme_id = '{cg_scheme_id}' and group_id = '{group_id}')")
            ...
            if not previous_group:
                self.cur_seqdef.execute(
                    f"INSERT INTO classification_group_profiles (cg_scheme_id, group_id, profile_id, scheme_id, curator, datestamp)"
                ...
                self.cur_seqdef.execute(
                    f"UPDATE classification_group_profiles SET group_id = '{group_id}' WHERE cg_scheme_id = '{cg_scheme_id}' AND \
    ...
    def _check_for_classification_schemes(self):
        ...
        if not query_res:
            ...
            for idx, threshold in enumerate(self.clustering_thresholds):
                ...
                self.cur_seqdef.execute(
                    f"INSERT INTO classification_schemes (id, scheme_id, name, description, inclusion_threshold, use_relative_threshold, display_order, status, curator, datestamp)"
