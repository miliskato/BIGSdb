...
schemedict = {'salmonella_mlst': {'dirdb': '/db/sequence_typing/salmonella/mlst', 'tsvname': 'mlst'},
              ...
              # 'salmonella_serogroup': {'dirdb': '/db/sequence_typing/salmonella/serogroup', 'tsvname': 'pcr_serogroup'},
              # 'salmonella_metal_detergent_resistance': {'dirdb': '/db/sequence_typing/salmonella/metal_detergent_resistance', 'tsvname': 'metal_detergent'},
              # 'salmonella_typing_virulence': {'dirdb': '/db/sequence_typing/salmonella/virulence', 'tsvname': 'typing_virulence'},
              # 'salmonella_antibiotic_resistance': {'dirdb': '/db/sequence_typing/salmonella/antibiotic_resistance', 'tsvname': 'typing_amr'},
              # 'salmonella_species_confirmation': {'dirdb': '/db/sequence_typing/salmonella/species_confirmation', 'tsvname': 'species_confirmation'}
              }
...
#                      'salmonella_virulencefinder': {
#                          'clusteredfasta': '/db/gene_detection/VirulenceFinder-salmonella/virulencefinder-salmonella-clustered_80.fasta',
#                          'metadatafile': '/db/gene_detection/VirulenceFinder-salmonella/mapping_full.json',
#                          'tsvname': 'hits_virulencefinder',
#                          'schemename_bigsdb': 'VirulenceFinder_salmonella',
#                          'schemename_html': 'VirulenceFinder - <i>salmonella</i>'},
#                      'salmonella_vfdbcore': {
    ...
    for scheme in schemedict:
        ...
        for dir in dirs:
            if not dir.startswith('.') and dir not in dirlist:
                ...
                # the elif below is specific to salmonella pcr serogroup where 0's are included in the profiles
                # (absent loci are required to define profiles)
                # Bigsdb creates a null allele itself in the seqdef database
                # elif result[2] == '-' and result[3] == '-' and dir in next(os.walk(schemedict['salmonella_serogroup']['dirdb']))[1]:
                #     allele_id = 0
                #     cur.execute(f"INSERT INTO allele_designations(locus, isolate_id, "
                #                 f"allele_id, status, method, sender, "
                #                 f"curator, date_entered, datestamp) "
...
#                 eavhtmltable = eavhtmltable + ''.join(['<td><a href="/galaxyreports/salmonella/', isolate_name, '/report.html#', genedetectiondict[scheme]['schemename_html'], '" target="_blank">', (json.loads(listofhits))[y][1], '</a></td></tr>'])
#             else:
#                 eavhtmltable = eavhtmltable + ''.join(['<td><a href="/galaxyreports/salmonella/', isolate_name, '/report.html#', genedetectiondict[scheme]['schemename_html'], '" target="_blank">', (json.loads(listofhits))[y][-2], '</a></td></tr>'])
#
