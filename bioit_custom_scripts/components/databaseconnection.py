...
class DatabaseConnection:
    ...
    def __init__(self, species: str, db_type: str) -> None:
        ...
        """
        if db_type != 'seqdef' and db_type != 'isolates':
            raise KeyError('no such database type')
        # Read the global config
        bigsdb_config_data = get_bigsdb_config_data()
        try:
            self.connection: psycopg2.extensions.connection = psycopg2.connect(database=f"bigsdb_{species}_{db_type}", user="apache", password=bigsdb_config_data.get('postgresql_apache_pass'),
                                   host="127.0.0.1", port="")
        except Exception:
