        ...
        with TblAlleleDesignations(self._species) as self._isolates_ad_psql_tbl, TblEavText(self._species) as self._isolates_eavt_psql_tbl:
            ...
            for scheme in self._schemedict:
                if scheme in self._sample_output_dict:
                    if self._schemedict[scheme]['type'] == 'regular':
                        for locus in self._sample_output_dict[scheme]['loci']:
                            if locus['Locus'] not in locusset:
                                if locus['% Identity'] == '100.00' and locus['HSP/Locus length'] != '-' and float(locus['HSP/Locus length']) == 1.0 and locus['Allele'] != 0 and locus['Allele'] != '?':
                                    self._isolates_ad_psql_tbl.insert_designation((locus['Locus'].replace("'", ""), self._isolatename, locus['Allele']))
                    ...
                    elif self._schemedict[scheme]['type'] == 'irregular':
                                        ...
                                        for antibiotic in antibiotics:
                                            ...
                                            eavhtmltable = eavhtmltable + ''.join(
                                                [f'<tr><td><a href="/galaxyreports/{self._species}/', self._isolatename,
                                                 '/report.html#',
                                                 self._schemedict[scheme]['schemename_html'], '" target="_blank">',
                                            ...
                                            self._isolates_ad_psql_tbl.insert_designation((antibiotic_reformatted, self._isolatename, mutation))
                        ...
                        if self._species == 'mycobacterium':
                            if scheme == 'spoligotyping':
                                ...
                                self._isolates_eavt_psql_tbl.insert_eav_isolate((self._isolatename, 'spoligotype_binary', self._sample_output_dict[scheme]['spoligotype_binary']))
                                self._isolates_eavt_psql_tbl.insert_eav_isolate((self._isolatename, 'spoligotype_octal', self._sample_output_dict[scheme]['spoligotype_octal']))
                            elif scheme == 'csb_rd':
                                for record in ['csb_detected', 'RD1_detected', 'RD9_detected']:
                                    ...
                                    allele_id = '1' if self._sample_output_dict[scheme][record] else '0'
                                ...
                                for bigsdbname, jsonname in amr_metadata_fields_tsv.items():
                                    self._isolates_eavt_psql_tbl.insert_eav_isolate((self._isolatename, bigsdbname, self._sample_output_dict[scheme][jsonname]))
                                # AMR results
                                with TblSchemeMembers(self._species) as isolates_schememembers_psql_tbl:
                                    ...
                                    if self._sample_output_dict[scheme][jsonname] != '-':
                                        ...
                                        for variant in self._sample_output_dict[scheme][jsonname].split(', '):
                                            ...
                                            if variantreformatted not in variantsset:
                                                self._isolates_ad_psql_tbl.insert_designation((locus[0], self._isolatename, variantreformatted))
                                            ...
                                            for antibiotic in ['rifampicin_SIR', 'penicillin_SIR']:
                                                if antibiotic in json_data['linked_data']['PubMLST isolates']:
                                                    for record in json_data['linked_data']['PubMLST isolates'][antibiotic]:
                                                        self._isolates_eavt_psql_tbl.insert_eav_isolate((self._isolatename, '_'.join([antibiotic, record['value'], 'frequency']),
                                                                              record['frequency']))
                            ...
                            if scheme == 'genotyphi':
                                    ...
                                    if item[0] in self._sample_output_dict[scheme]['results'] and self._sample_output_dict[scheme]['results'][item[0]] is not None:
                                        ...
                                        for value in future_alleles:
                                            if value != '-':
                                                ...
                                                self._isolates_ad_psql_tbl.insert_designation((genotyphi_field, self._isolatename, value))
                            elif scheme == 'sistr':
                                ...
                                if serotyping_insert != '-':
                                    ...
                                    self._isolates_eavt_psql_tbl.insert_eav_isolate((self._isolatename, f'{scheme}_formula', serotyping_insert))
                                serotyping_insert = self._sample_output_dict[scheme]['serotype_concensus']
                                if serotyping_insert != '-':
                                    self._isolates_eavt_psql_tbl.insert_eav_isolate((self._isolatename, f'{scheme}_serotype', serotyping_insert))
                                ...
                                if len(hits) != 0:
                                    ...
                                    for spi in hits:
                                        ...
                                        if spifinder_entry not in inserted_alleledesignations_list:
                                            ...
                                            self._isolates_ad_psql_tbl.insert_designation((spifinder_field, self._isolatename, spifinder_entry))
