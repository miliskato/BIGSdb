...
schemedict = {'listeria_ndaro':           {'clusteredfasta': '/db/gene_detection/NCBI_AMR/ncbi_amr-clustered_80.fasta',
              ...
              'salmonella_resfinder':            {'clusteredfasta': '/db/gene_detection/ResFinder/resfinder-clustered_80.fasta',
                                            ...
                                            'seqdefdb': 'bigsdb_salmonella_seqdef',
                                            'species': 'salmonella'},
              'salmonella_plasmidfinder':   {'clusteredfasta': '/db/gene_detection/PlasmidFinder-entero/plasmidfinder-entero-clustered_80.fasta',
                                            'metadatafile': '/db/gene_detection/PlasmidFinder-entero/mapping_full.json',
                                            'schemename_bigsdb': 'PlasmidFinder_entero',
                                            'schemename_html': 'PlasmidFinder - Enterobacteriaceae',
                                            'isolatedb': 'bigsdb_salmonella_isolates',
...
for scheme in schemedict:
    ...
    for x in list(sequencefile):
        ...
        if scheme != 'vfdb_core':
            ...
            descriptiondict['_'.join([schemedict[scheme]['schemename_bigsdb'], (sequencefile[x]['cluster'])])].append((sequencefile[x]['gene']).replace("'", ""))
    clusterfile = open(Path(schemedict[scheme]['clusteredfasta']), 'r').readlines()
    clusterdict = {}
    for line in clusterfile:
        # line looks like this: >0__Cluster_0__seq_4648__seq_4648
    ...
    cur = con.cursor()
    con.autocommit = True
    cur.execute(f"DELETE FROM allele_designations WHERE locus LIKE '{schemedict[scheme]['schemename_bigsdb']}_Cluster%'")
