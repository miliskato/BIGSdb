...
function get_jwt_preview(id, species, validation_type, res_time, get_zip, dtap, newWindow){
    query_string = "http://" + HOSTANDPORT + "/search?isolate_id=" + id + '&res_version=' + res_time +"&species=" + species + "&get_zip=" + get_zip + "&dtap=" + dtap + "&validation_type=" + validation_type
...
function get_jwt_report (get_zip, validation_type_opt, id_opt, species_opt, res_version_opt){
    if (arguments.length === 1) {
        ...
        var res_version = res_version_opt
    }
    ...
    dt.forEach((el, index) => {
     ...
     $.ajax({
        ...
        complete: function(){
            if(get_zip === 'yes'){
                get_jwt_zip(id, species, validation_type, res_version, get_zip, dtap, newWindow)
            }else{
                 get_jwt_preview(id, species, validation_type, res_version, get_zip, dtap, newWindow)
            }
        },
        dataType: 'json'
    });
